...
def project_electrodes_anydirection(tri, vert, elecmatrix, proj_direction):
    ...
    Parameters
    ----------
    cortex:       cortical surface struct (with fields cortex.vert
               and cortex.tri)
    ...
    -------
    elecs_proj:
        A [nchans] x 3 position array of locations of
    ...
    for i in range(elecmatrix.shape[0]):  # Loop through all electrodes
        ...
        elec_intersect, _, _, _, xcoor = TriangleRayIntersection(
        ...
        if np.sum(elec_intersect) > 1:
            ...
            x = xctmp[
                np.argmin(np.sum(np.abs(orig - xctmp) ** 2, axis=1) ** (1.0 / 2)), :
            ]
            elecs_proj[i, :] = x
...
def TriangleRayIntersection(
    ...
    #      triangle is defined by vertex points: VERT1, VERT2, VERT3. All input
    ...
    orig, direction, vert0, vert1, vert2 = (
        np.array(orig),
        np.array(direction),
        np.array(vert0),
        np.array(vert1),
    ...
    if fullReturn:
        ...
        v[ok] = (
            np.sum(direction[ok, :] * qvec, axis=1) / det[ok]
